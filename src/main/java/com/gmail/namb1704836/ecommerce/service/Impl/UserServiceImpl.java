...
import com.gmail.namb1704836.ecommerce.dto.PasswordResetDto;
import com.gmail.namb1704836.ecommerce.repository.PerfumeRepository;
import com.gmail.namb1704836.ecommerce.repository.ReviewRepository;
...
/**
 * The service layer class implements the accessor methods of {@link User}
 * objects in the {@link UserService} interface database. The class is marked
...
public class UserServiceImpl implements UserDetailsService, UserService {
	/**
	 * Implementation of the {@link UserRepository} interface for working with users
	 * with a database.
	 */
	private final UserRepository userRepository;

	/**
	 * Implementation of the {@link MailSender} class for working with email.
	 */
	private final MailSender mailSender;

	/**
	 * Implementation of the {@link PasswordEncoder} interface for encoding
	 * passwords.
	 */
	 ...
	 * objects.
	 * 
	 * @param userRepository    implementation of the {@link UserRepository}
	 *                          interface for working with users with a database.
	 * @param mailSender        implementation of the {@link MailSender} class for
	 *                          working with email.
	 * @param passwordEncoder   implementation of the {@link PasswordEncoder}
	 *                          interface
	...
	public UserServiceImpl(UserRepository userRepository, MailSender mailSender, PasswordEncoder passwordEncoder,
			PerfumeRepository perfumeRepository, ReviewRepository reviewRepository) {
	...
	/**
	 * Returns the user that has the same activation code as the value of the input
	 * parameter.
	 *
	...
	/**
	 * Returns the user that has the same password reset code as the value of the
	 * input parameter.
	 *
	 ...
	 * @throws LockedException           if an authentication request is rejected
	 ...
	 *
	 * @param user          the user to whom a message with an activation/reset code
	...
	public void sendMessage(User user, List<String> emailMessages, String subject, String code, String urlPart) {
		if (!StringUtils.isEmpty(user.getEmail())) {
			String message = String.format("Hello, %s! \n" + "%s \n" + "%s http://%s/%s/%s", user.getUsername(),
					emailMessages.get(0), emailMessages.get(1), hostname, urlPart, code);
